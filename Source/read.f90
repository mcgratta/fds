...
CALL READ_HOOD    ; CALL CHECK_STOP_STATUS ; IF (STOP_STATUS/=NO_STOP) RETURN
...
!> \brief Read the HOOD namelist lines

SUBROUTINE READ_HOOD

INTEGER :: NN,N_HOODS_NEW,N,II,JJ,KK,N_HOODS_READ,NNN
REAL(EB) :: XB1,XB2,XB3,XB4,XB5,XB6
CHARACTER(LABEL_LENGTH) :: MULT_ID
NAMELIST /HOOD/ MULT_ID,XB
...
REWIND(LU_INPUT) ; INPUT_FILE_LINE_NUMBER = 0
COUNT_HOOD_LOOP: DO
   CALL CHECKREAD('HOOD',LU_INPUT,IOS)  ; IF (STOP_STATUS==SETUP_STOP) RETURN
   IF (IOS==1) EXIT COUNT_HOOD_LOOP
   MULT_ID = 'null'
   READ(LU_INPUT,HOOD,END=15,ERR=16,IOSTAT=IOS)
   N_HOODS_READ = N_HOODS_READ + 1
   N_HOODS_NEW = 0
   ...
   ELSE
      DO N=1,N_MULT
         MR => MULTIPLIER(N)
         IF (MULT_ID==MR%ID) N_HOODS_NEW = MR%N_COPIES
      ENDDO
      IF (N_HOODS_NEW==0) THEN
         WRITE(MESSAGE,'(A,A,A,I0)') 'ERROR: MULT line ', TRIM(MULT_ID),' not found on HOOD line', N_HOODS_READ
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
   ENDIF
   N_HOODS = N_HOODS + N_HOODS_NEW
16 IF (IOS>0) THEN
      WRITE(MESSAGE,'(A,I0)') 'ERROR: Problem with HOOD line, line number ',INPUT_FILE_LINE_NUMBER
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF

ENDDO COUNT_HOOD_LOOP
15 CONTINUE
...
ALLOCATE(HOODS(N_HOODS),STAT=IZERO) ; CALL ChkMemErr('READ','MESHES',IZERO)

! Read in the HOOD lines from Input file

REWIND(LU_INPUT) ; INPUT_FILE_LINE_NUMBER = 0

NN = 0

HOOD_LOOP: DO N=1,N_HOODS_READ
...
INTEGER :: IJK(3),NH,NM,NM2,CURRENT_MPI_PROCESS,MPI_PROCESS,RGB(3),N_MESH_NEW,N,II,JJ,KK,NMESHES_READ,NNN,JBAR_OLD_VALUE
...
NAMELIST /PART/ ADHERE_TO_SOLID,AGE,BREAKUP,BREAKUP_CNF_RAMP_ID,BREAKUP_DISTRIBUTION,BREAKUP_GAMMA_D,BREAKUP_RATIO,&
                ...
                SURFACE_DIAMETER,SURFACE_TENSION,SURF_ID,TURBULENT_DISPERSION,TRACK_EMBERS,VERTICAL_VELOCITY
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   SF%EMBER_POWER_MEAN = EMBER_POWER_MEAN
   SF%EMBER_POWER_SIGMA = EMBER_POWER_SIGMA
...
EMBER_POWER_MEAN        = -1._EB
EMBER_POWER_SIGMA       = 0.001_EB
EMISSIVITY              = -1._EB
      ...
      IF ((ABS(VT%IOR)==1 .AND. I1>=1 .AND. I1<=IBM1) .OR. &
         ...
         IF (.NOT.SOLID_FOUND) THEN
            WRITE(MESSAGE,'(A,A,A,I0)') &
               'ERROR: VENT ',TRIM(VT%ID),' has no solid backing or an orientation index (IOR) is needed. MESH number=',NM
            CALL SHUTDOWN(MESSAGE,PROCESS_0_ONLY=.FALSE.) ; RETURN
         ENDIF
...
COUNT_LOOP: DO
   ...
   12 IF (IOS>0) THEN
      WRITE(MESSAGE,'(A,I0,A,I0)') 'ERROR: Problem with INIT number ',N_INIT_READ,', line number ',INPUT_FILE_LINE_NUMBER
      CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
   N_INIT_NEW = 0
   IF (MULT_ID=='null') THEN
               ...
               IF (TRIM(ORIENTATION_RAMP(1))/='null' .OR. TRIM(ORIENTATION_RAMP(2))/='null' .OR. &
                  ...
                  IF (TRIM(ORIENTATION_RAMP(1))/='null') THEN
                     ...
                     IF (IN%ORIENTATION_RAMP_INDEX(1)==0) THEN
                        WRITE(MESSAGE,'(A,I0,A,A)') 'ERROR: Problem with INIT number ',N,'. ORIENTATION_RAMP(1) not found.'
                        CALL SHUTDOWN(MESSAGE) ; RETURN
                     ENDIF
                  ENDIF
                  IF (TRIM(ORIENTATION_RAMP(2))/='null') THEN
                     ...
                     IF (IN%ORIENTATION_RAMP_INDEX(2)==0) THEN
                        WRITE(MESSAGE,'(A,I0,A,A)') 'ERROR: Problem with INIT number ',N,'. ORIENTATION_RAMP(2) not found.'
                        CALL SHUTDOWN(MESSAGE) ; RETURN
                     ENDIF
                  ENDIF
                  IF (TRIM(ORIENTATION_RAMP(3))/='null') THEN
                     ...
                     IF (IN%ORIENTATION_RAMP_INDEX(3)==0) THEN
                        WRITE(MESSAGE,'(A,I0,A,A)') 'ERROR: Problem with INIT number ',N,'. ORIENTATION_RAMP(3) not found.'
                        CALL SHUTDOWN(MESSAGE) ; RETURN
                     ENDIF
                  ENDIF
                  IF (ANY(IN%ORIENTATION_RAMP_INDEX <= 0)) THEN
                     WRITE(MESSAGE,'(A,I0,A,A)') 'ERROR: Problem with INIT number ',N,&
                                                 '. If any ORIENTATION_RAMP is defined, then all must be defined.'
                     CALL SHUTDOWN(MESSAGE) ; RETURN
                  ENDIF
               ENDIF
...
READ_DEVC_LOOP: DO NN=1,N_DEVC_READ
   ...
   IF (POINTS>1 .AND. ANY(XBP<-1.E5_EB) .AND. INIT_ID=='null') THEN
      WRITE(MESSAGE,'(A,A,A)')  'ERROR: DEVC ',TRIM(ID),' must have coordinates given in terms of XBP'
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
   ...
   POINTS_LOOP: DO I_POINT=1,POINTS
      ...
      IF (DV%QUANTITY(1)=='RADIATIVE HEAT FLUX GAS' .OR. &
         ...
         IF (DV%ORIENTATION_INDEX==0) THEN
            WRITE(MESSAGE,'(A,A,A)')  'ERROR: DEVC ',TRIM(ID),' must have an ORIENTATION'
            CALL SHUTDOWN(MESSAGE) ; RETURN
         ENDIF
         DV%INIT_ID = DV%ID
...
IF (READ_EXTERNAL .AND. EXTERNAL_FILENAME=='null') THEN
   WRITE(MESSAGE,'(A)') 'ERROR: A CTRL FUNCTION_TYPE=EXTERNAL is present but no EXTERNAL_FILENAME is defined on MISC/'
   CALL SHUTDOWN(MESSAGE) ; RETURN
ENDIF
...
PROF_LOOP: DO NN=1,N_PROFO
   ...
   IF (.NOT. OUTPUT_QUANTITY(PF%QUANTITY_INDEX)%PROF_APPROPRIATE) THEN
      WRITE(MESSAGE,'(A,I0,A,A,A)') 'ERROR: PROF ' ,NN,' QUANTITY of ',TRIM(PF%QUANTITY),' is not a valid QUANTITY for PROF.'
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
   PF%X            = XYZ(1)
   PF%Y            = XYZ(2)
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   SLCF_LOOP: DO NN=1,N_SLCF_O
      ...
      IF (AGL_SLICE>=0._EB .AND. CELL_CENTERED) THEN
         WRITE(MESSAGE,'(A)') "ERROR: CELL_CENTERED not allowed with AGL_SLICE"
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
...
COUNT_SM3D_LOOP: DO
   ...
   210 IF (IOS>0) THEN
         WRITE(MESSAGE,'(A,I0,A,I0)') 'ERROR: Problem with SM3D number ',N_SMOKE3D+1,', line number ',INPUT_FILE_LINE_NUMBER
         CALL SHUTDOWN(MESSAGE) ; RETURN
       ENDIF
