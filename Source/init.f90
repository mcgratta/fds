...
USE CONTROL_VARIABLES
INTEGER :: N,I,J,K,IPTS,JPTS,KPTS,IW,IC,IERR,IPZ,IZERO,ICF,NSLICE
INTEGER, INTENT(IN) :: NM
...
ALLOCATE(M%WALL_WORK1(M%N_WALL_CELLS_DIM),STAT=IZERO)
CALL ChkMemErr('INIT','WALL_WORK1',IZERO)
ALLOCATE(M%WALL_WORK2(M%N_WALL_CELLS_DIM),STAT=IZERO)
...
M%N_EDGES_DIM = 4*(IBP1*JBP1+IBP1*KBP1+JBP1*KBP1)
DO N=1,M%N_OBST
   OB=>M%OBSTRUCTION(N)
   IPTS = OB%I2-OB%I1
   JPTS = OB%J2-OB%J1
   KPTS = OB%K2-OB%K1
   M%N_EDGES_DIM = M%N_EDGES_DIM + 4*(IPTS*JPTS+IPTS*KPTS+JPTS*KPTS)
ENDDO

CALL REALLOCATE_EDGE(NM,M%N_EDGES_DIM,M%N_EDGES_DIM)
...
! Arguments for DEFINE_EDGE(I,J,K,IOR,IEC,NM)

DO K=0,M%KBAR
   DO J=0,M%JBAR
      IF (J>0) CALL DEFINE_EDGE(     0,J,K, 1,2,NM)
      IF (J>0) CALL DEFINE_EDGE(M%IBAR,J,K,-1,2,NM)
      IF (K>0) CALL DEFINE_EDGE(     0,J,K, 1,3,NM)
      IF (K>0) CALL DEFINE_EDGE(M%IBAR,J,K,-1,3,NM)
   ENDDO
ENDDO
DO K=0,M%KBAR
   DO I=0,M%IBAR
      IF (I>0) CALL DEFINE_EDGE(I,     0,K, 2,1,NM)
      IF (I>0) CALL DEFINE_EDGE(I,M%JBAR,K,-2,1,NM)
      IF (K>0) CALL DEFINE_EDGE(I,     0,K, 2,3,NM)
      IF (K>0) CALL DEFINE_EDGE(I,M%JBAR,K,-2,3,NM)
   ENDDO
ENDDO
DO J=0,M%JBAR
   DO I=0,M%IBAR
      IF (I>0) CALL DEFINE_EDGE(I,J,     0, 3,1,NM)
      IF (I>0) CALL DEFINE_EDGE(I,J,M%KBAR,-3,1,NM)
      IF (J>0) CALL DEFINE_EDGE(I,J,     0, 3,2,NM)
      IF (J>0) CALL DEFINE_EDGE(I,J,M%KBAR,-3,2,NM)
   ENDDO
ENDDO
...
OBST_LOOP_3: DO N=1,M%N_OBST
   ...
   DO K=OB%K1,OB%K2
      DO J=OB%J1,OB%J2
         IF (J>OB%J1) CALL DEFINE_EDGE(OB%I1,J,K,-1,2,NM)
         IF (J>OB%J1) CALL DEFINE_EDGE(OB%I2,J,K, 1,2,NM)
         IF (K>OB%K1) CALL DEFINE_EDGE(OB%I1,J,K,-1,3,NM)
         IF (K>OB%K1) CALL DEFINE_EDGE(OB%I2,J,K, 1,3,NM)
      ENDDO
   ENDDO
   DO K=OB%K1,OB%K2
      DO I=OB%I1,OB%I2
         IF (I>OB%I1) CALL DEFINE_EDGE(I,OB%J1,K,-2,1,NM)
         IF (I>OB%I1) CALL DEFINE_EDGE(I,OB%J2,K, 2,1,NM)
         IF (K>OB%K1) CALL DEFINE_EDGE(I,OB%J1,K,-2,3,NM)
         IF (K>OB%K1) CALL DEFINE_EDGE(I,OB%J2,K, 2,3,NM)
      ENDDO
   ENDDO
   DO J=OB%J1,OB%J2
      DO I=OB%I1,OB%I2
         IF (I>OB%I1) CALL DEFINE_EDGE(I,J,OB%K1,-3,1,NM)
         IF (I>OB%I1) CALL DEFINE_EDGE(I,J,OB%K2, 3,1,NM)
         IF (J>OB%J1) CALL DEFINE_EDGE(I,J,OB%K1,-3,2,NM)
         IF (J>OB%J1) CALL DEFINE_EDGE(I,J,OB%K2, 3,2,NM)
      ENDDO
   ENDDO
ENDDO OBST_LOOP_3
...
SUBROUTINE DEFINE_EDGE(II,JJ,KK,IOR,IEC,NM)

USE MEMORY_FUNCTIONS, ONLY: REALLOCATE_EDGE
INTEGER, INTENT(IN) :: II,JJ,KK,IOR,IEC,NM
INTEGER :: NOM,ICMM,ICMP,ICPM,ICPP,IE,IW,IIO,JJO,KKO,IW1,IW2
REAL(EB) :: XI,YJ,ZK
...
IF (IE==0) THEN
   ...
   IE = EDGE_COUNT(NM)
   IF (IE>M%N_EDGES_DIM) CALL REALLOCATE_EDGE(NM,M%N_EDGES_DIM,M%N_EDGES_DIM+50)
ENDIF
...
IF ( (II==0      .AND. KK==0     ) .OR. &
     (II==0      .AND. KK==M%KBAR) .OR. &
     (II==M%IBAR .AND. KK==0     ) .OR. &
     (II==M%IBAR .AND. KK==M%KBAR) .OR. &
     (II==0      .AND. JJ==0     ) .OR. &
     (II==0      .AND. JJ==M%JBAR) .OR. &
     (II==M%IBAR .AND. JJ==0     ) .OR. &
     (II==M%IBAR .AND. JJ==M%JBAR) .OR. &
     (JJ==0      .AND. KK==0     ) .OR. &
     (JJ==0      .AND. KK==M%KBAR) .OR. &
     (JJ==M%JBAR .AND. KK==0     ) .OR. &
     (JJ==M%JBAR .AND. KK==M%KBAR) ) ED%EXTERNAL=.TRUE.
...
INTEGER, INTENT(IN) :: NM
REAL(EB), INTENT(IN) :: T
INTEGER :: I1,I2,J1,J2,K1,K2,I,J,K,IW,ICG,IC,OBST_INDEX,NOM,IIO,JJO,KKO,IOR
REAL(EB) :: TNOW
LOGICAL :: CREATE,REMOVE
TYPE (OBSTRUCTION_TYPE), POINTER :: OB
TYPE (WALL_TYPE), POINTER :: WC
...
! Loop over external boundary of mesh NM and look for wall cells that abut another mesh, NOM.
! Get the boundary type of the cell which is not attached to an obstruction. 
...
OBSTRUCTION_LOOP: DO OBST_INDEX=1,N_OBST
   ...
   DO K=K1+1,K2
      DO J=J1+1,J2
         I   = I1+1
         IC  = CELL_INDEX(I  ,J,K)
         ICG = CELL_INDEX(I-1,J,K)
         IOR = -1
         IW  = CELL(ICG)%WALL_INDEX( 1)
         IF (I>1)    CALL GET_BOUNDARY_TYPE(IW)
         I   = I1
         IC  = CELL_INDEX(I  ,J,K)
         ICG = CELL_INDEX(I+1,J,K)
         IOR = 1
         IW  = CELL(ICG)%WALL_INDEX(-1)
         IF (I<IBAR) CALL GET_BOUNDARY_TYPE(IW)
         I   = I2
         IC  = CELL_INDEX(I  ,J,K)
         ICG = CELL_INDEX(I+1,J,K)
         IOR = 1
   ...
   DO K=K1+1,K2
      DO I=I1+1,I2
         J   = J1+1
         IC  = CELL_INDEX(I,J  ,K)
         ICG = CELL_INDEX(I,J-1,K)
         IOR = -2
         IW  = CELL(ICG)%WALL_INDEX( 2)
         IF (J>1)    CALL GET_BOUNDARY_TYPE(IW)
         J   = J1
         IC  = CELL_INDEX(I,J  ,K)
         ICG = CELL_INDEX(I,J+1,K)
         IOR = 2
         IW  = CELL(ICG)%WALL_INDEX(-2)
         IF (J<JBAR) CALL GET_BOUNDARY_TYPE(IW)
         J   = J2
         IC  = CELL_INDEX(I,J  ,K)
         ICG = CELL_INDEX(I,J+1,K)
         IOR = 2
   ...
   DO J=J1+1,J2
      DO I=I1+1,I2
         K   = K1+1
         IC  = CELL_INDEX(I,J,K  )
         ICG = CELL_INDEX(I,J,K-1)
         IOR = -3
         IW  = CELL(ICG)%WALL_INDEX( 3)
         IF (K>1)    CALL GET_BOUNDARY_TYPE(IW)
         K   = K1  
         IC  = CELL_INDEX(I,J,K  )
         ICG = CELL_INDEX(I,J,K+1)
         IOR = 3
         IW  = CELL(ICG)%WALL_INDEX(-3)
         IF (K<KBAR) CALL GET_BOUNDARY_TYPE(IW)
         K   = K2
         IC  = CELL_INDEX(I,J,K  )
         ICG = CELL_INDEX(I,J,K+1)
         IOR = 3
   ...
   ! If uninitialized edges are exposed when the obstruction is removed, define them.
   
   IF (REMOVE) THEN
   
      DO K=K1,K2
         DO J=J1,J2
            IF (J>J1) CALL REDEFINE_EDGE(I1,J,K,2, 1)
            IF (J>J1) CALL REDEFINE_EDGE(I2,J,K,2,-1)
            IF (K>K1) CALL REDEFINE_EDGE(I1,J,K,3, 1)
            IF (K>K1) CALL REDEFINE_EDGE(I2,J,K,3,-1)
         ENDDO
      ENDDO
      
      DO K=K1,K2
         DO I=I1,I2
            IF (I>I1) CALL REDEFINE_EDGE(I,J1,K,1, 2)
            IF (I>I1) CALL REDEFINE_EDGE(I,J2,K,1,-2)
            IF (K>K1) CALL REDEFINE_EDGE(I,J1,K,3, 2)
            IF (K>K1) CALL REDEFINE_EDGE(I,J2,K,3,-2)
         ENDDO
      ENDDO
      
      DO J=J1,J2
         DO I=I1,I2
            IF (I>I1) CALL REDEFINE_EDGE(I,J,K1,1, 3)
            IF (I>I1) CALL REDEFINE_EDGE(I,J,K2,1,-3)
            IF (J>J1) CALL REDEFINE_EDGE(I,J,K1,2, 3)
            IF (J>J1) CALL REDEFINE_EDGE(I,J,K2,2,-3)
         ENDDO
      ENDDO
...
!> \param IW_LOCAL Local value of the wall cell index

SUBROUTINE GET_BOUNDARY_TYPE(IW_LOCAL)

INTEGER :: IIG,JJG,KKG,IW_OLD,IERR,PRESSURE_BC_TYPE,ICG_OLD,II,IW_LOCAL
...
IF (IW_LOCAL==0) THEN
   ...
   N_INTERNAL_WALL_CELLS = N_INTERNAL_WALL_CELLS + 1
   N_WALL_CELLS = N_EXTERNAL_WALL_CELLS + N_INTERNAL_WALL_CELLS
   IW_LOCAL  = N_WALL_CELLS
   CALL INIT_WALL_CELL(NM,I,J,K,CELL(IC)%OBST_INDEX,IW_LOCAL,IOR,OB%SURF_INDEX_INTERIOR,IERR)
   IF (IERR>0) RETURN
ENDIF
...
IF (IW_LOCAL<=N_EXTERNAL_WALL_CELLS .AND. REMOVE) THEN
   EWC => EXTERNAL_WALL(IW_LOCAL)
...
IF (IW_LOCAL>N_EXTERNAL_WALL_CELLS) THEN
...
IF (WC%BOUNDARY_TYPE/=NULL_BOUNDARY) THEN
   ...
   CALL INIT_WALL_CELL(NM,BC%II,BC%JJ,BC%KK,WC%OBST_INDEX,IW_LOCAL,BC%IOR,WC%SURF_INDEX,IERR)
   CALL SET_DENSITY_AND_MASS_FRACTIONS_AT_WALL(NM,IW_LOCAL,T)
   WC => WALL(IW_LOCAL)
   IF (IW_LOCAL<=N_EXTERNAL_WALL_CELLS) EWC%PRESSURE_BC_TYPE = PRESSURE_BC_TYPE
ENDIF
...
USE_OLD_WALL_CELL: IF (REMOVE .AND. ( (SF%THERMAL_BC_INDEX==THERMALLY_THICK.AND.(SF%VARIABLE_THICKNESS.OR.SF%HT_DIM>1)) &
   ...
   IF (CELL(ICG_OLD)%EXTERIOR) THEN
      EWC => EXTERNAL_WALL(CELL(ICG)%WALL_INDEX(BC%IOR))
      NOM = EWC%NOM
      IF (NOM>0) THEN
         ...
         IW_OLD = MESHES(NOM)%CELL(ICG_OLD)%WALL_INDEX(-BC%IOR)
      ELSE
         IW_OLD = 0
      ENDIF
   ELSE
      NOM = NM
      IW_OLD = CELL(ICG_OLD)%WALL_INDEX(-BC%IOR)
   ENDIF
   ...
   SWAP: IF (IW_OLD>0) THEN
      ...
      ELSEIF (.NOT.MESHES(NOM)%CELL(ICG_OLD)%SOLID .AND. .NOT.MESHES(NM)%CELL(ICG)%SOLID .AND. MESHES(NM)%CELL(IC)%SOLID .AND. &
         ...
         IF (ONE_D_OLD%BACK_MESH>0 .AND. ONE_D_OLD%BACK_MESH/=NM) THEN
            ...
            DO II=1,OS%N_ITEMS
               IF (OS%ITEM_INDEX(II)==IW_OLD) OS%ITEM_INDEX(II) = IW_LOCAL
            ENDDO
         ENDIF
      ENDIF
   ENDIF SWAP
...
!> \param IOR Orientation index

SUBROUTINE REDEFINE_EDGE(II,JJ,KK,IEC,IOR)

INTEGER :: IE,II,JJ,KK,IEC,IOR

SELECT CASE(IEC)
   CASE(1) ; IE = CELL(CELL_INDEX(II,JJ,KK))%EDGE_INDEX( 4)
   CASE(2) ; IE = CELL(CELL_INDEX(II,JJ,KK))%EDGE_INDEX( 8)
   CASE(3) ; IE = CELL(CELL_INDEX(II,JJ,KK))%EDGE_INDEX(12)
END SELECT

IF (IE==0) CALL DEFINE_EDGE(II,JJ,KK,IOR,IEC,NM)
