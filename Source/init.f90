...
OBST_SHAPE_IF: IF (OBST_SHAPE_AREA_ADJUST) THEN
   ...
   SHAPE_LOOP_1: DO IW=M%N_EXTERNAL_WALL_CELLS+1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
      ...
      I_OBST=M%CELL(IC)%OBST_INDEX; IF (I_OBST==0)       CYCLE SHAPE_LOOP_1
   ...
   SHAPE_LOOP_2: DO IW=M%N_EXTERNAL_WALL_CELLS+1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
      ...
      IOR=BC%IOR
      IC=M%CELL_INDEX(II,JJ,KK)
      I_OBST=M%CELL(IC)%OBST_INDEX; IF (I_OBST==0)       CYCLE SHAPE_LOOP_2
...
IF (ALLOCATED(ONE_D%M_DOT_S_PP)) DEALLOCATE(ONE_D%M_DOT_S_PP) ; ALLOCATE(ONE_D%M_DOT_S_PP(0:ONE_D%N_MATL))
...
ELSEIF (PRESENT(THIN_WALL_CELL)) THEN
   ...
   CALL PACK_THIN_WALL(NM,OS_DUMMY,TW,TW%SURF_INDEX,TW%N_REALS,TW%N_INTEGERS,UNPACK_IT=.FALSE.,COUNT_ONLY=.TRUE.)
ENDIF
...
IF (PRESENT(WALL_INDEX)) THEN
   ...
   IF (SF%NORMAL_DIRECTION_ONLY .AND. SF2%NORMAL_DIRECTION_ONLY) RETURN
...
INTEGER, INTENT(IN) :: NM
INTEGER :: III,N,II,JJ,KK,IOR,IW,SURF_INDEX,IIG,JJG,KKG,ICF,ICG
REAL(EB) :: DEPTH
...
DEVICE_LOOP: DO N=1,N_DEVC
   ...
   IF (DV%INIT_ID=='null') THEN ! Assume the device is tied to a WALL cell or CFACE
      ...
      IF (TRIM(DV%QUANTITY(1))=='SOLID CELL TEMPERATURE') THEN
         ...
         SELECT CASE (IOR)
            CASE ( 1); DO IIG=II,M%IBP1; IF (.NOT.M%CELL(ICG)%SOLID) EXIT; ENDDO
            CASE (-1); DO IIG=II,  0,-1; IF (.NOT.M%CELL(ICG)%SOLID) EXIT; ENDDO
            CASE ( 2); DO JJG=JJ,M%JBP1; IF (.NOT.M%CELL(ICG)%SOLID) EXIT; ENDDO
            CASE (-2); DO JJG=JJ,  0,-1; IF (.NOT.M%CELL(ICG)%SOLID) EXIT; ENDDO
            CASE ( 3); DO KKG=KK,M%KBP1; IF (.NOT.M%CELL(ICG)%SOLID) EXIT; ENDDO
            CASE (-3); DO KKG=KK,  0,-1; IF (.NOT.M%CELL(ICG)%SOLID) EXIT; ENDDO
         END SELECT
      ENDIF
...
IF (OBST_INDEX>0) THEN
   ...
   IF (OBX%I1==OBX%I2 .AND. OBX%THIN) THEN ; BC%X1=OBX%X1 ; BC%X2=OBX%X2 ; ENDIF
   IF (OBX%J1==OBX%J2 .AND. OBX%THIN) THEN ; BC%Y1=OBX%Y1 ; BC%Y2=OBX%Y2 ; ENDIF
   IF (OBX%K1==OBX%K2 .AND. OBX%THIN) THEN ; BC%Z1=OBX%Z1 ; BC%Z2=OBX%Z2 ; ENDIF
ENDIF
...
B1%RHO_F = M%RHO(IIG,JJG,KKG)
B1%RHO_D_F = 0._EB
B1%RHO_D_DZDN_F = 0._EB

IF (M%CELL(M%CELL_INDEX(I,J,K))%SOLID .OR. IW<=M%N_EXTERNAL_WALL_CELLS) THEN
...
IF (OBST_INDEX>0) THEN
   OBX=>M%OBSTRUCTION(OBST_INDEX)
   IF (OBX%FDS_AREA(ABS(IOR))>TWO_EPSILON_EB) B1%AREA_ADJUST = SF%AREA_MULTIPLIER*OBX%INPUT_AREA(ABS(IOR))/OBX%FDS_AREA(ABS(IOR))
   IF (B1%AREA_ADJUST<=TWO_EPSILON_EB) B1%AREA_ADJUST = 1._EB
   IF (OBX%MASS>1.E5_EB .AND. OBX%BULK_DENSITY<=0._EB) OBX%MASS = SF%SURFACE_DENSITY*B1%AREA*B1%AREA_ADJUST
ENDIF
...
IF (OBST_INDEX>0 .AND. ABS(SF%VOLUME_FLOW)>=TWO_EPSILON_EB) THEN
   ...
   IF (OBX%FDS_AREA(ABS(IOR))>TWO_EPSILON_EB) &
      B1%U_NORMAL_0 = SF%VOLUME_FLOW*(OBX%INPUT_AREA(ABS(IOR))/OBX%UNDIVIDED_INPUT_AREA(ABS(IOR))) / OBX%FDS_AREA(ABS(IOR))
ENDIF
...
PROCESS_VENT: IF (WC%VENT_INDEX>0) THEN
   ...
   IF (SF%PROFILE==ATMOSPHERIC_PROFILE) THEN
      IF (M%ZC(K)<GROUND_LEVEL) THEN
         ...
         RETURN
      ENDIF
      B1%U_NORMAL_0 = B1%U_NORMAL_0*((M%ZC(K)-GROUND_LEVEL)/SF%Z0)**SF%PLE
   ENDIF
...
IF (SURFACE(SURF_INDEX)%NORMAL_DIRECTION_ONLY) THEN
   M%N_THIN_WALL_CELLS = M%N_THIN_WALL_CELLS - 1
   RETURN
ENDIF
...
INTEGER :: II,JJ,KK,IC,ICG,IOR,NOM,ITER,OBST_INDEX,OBST_INDEX_PREVIOUS,NN,NNN,NL,N_LAYERS,N_MATLS,N_LAYERS_NEW,N_MATLS_NEW
...
REAL(EB) :: XXC,YYC,ZZC,THICKNESS,OLD_THICKNESS,TOTAL_THICKNESS,UPPER_BOUND,LOWER_BOUND,LINING_THICKNESS,BACK_LINING_THICKNESS
...
FIND_BACK_WALL_CELL: DO  ! Look for the back wall cell; that is, the wall cell on the other side of the obstruction
   ...
   IF (SF%VARIABLE_THICKNESS .OR. SF%HT_DIM>1) THEN 
      ...
      IF (ITER==1 .AND. OBST_INDEX<1) OBST_INDEX = WC%OBST_INDEX
      ...
      IF (OBST_INDEX>0) THEN
         DO NN=1,N_MATLS
            DO NNN=1,MAX_MATERIALS
                  IF (OB%MATL_INDEX(NNN)==MATL_INDEX(NN)) MATL_MASS_FRACTION(N_LAYERS,NN) = OB%MATL_MASS_FRACTION(NNN)
            ENDDO
         ENDDO
      ENDIF
