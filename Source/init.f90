...
DO K=K1+1,K2
   DO J=J1+1,J2
      ...
      IF (I>1)    CALL GET_BOUNDARY_TYPE
      I   = I1
      IC  = CELL_INDEX(I  ,J,K)
      ICG = CELL_INDEX(I+1,J,K)
      IOR = 1
      IW  = CELL(ICG)%WALL_INDEX(-1)
      IF (I<IBAR) CALL GET_BOUNDARY_TYPE
      I   = I2
      IC  = CELL_INDEX(I  ,J,K)
      ICG = CELL_INDEX(I+1,J,K)
      IOR = 1
...
DO K=K1+1,K2
   DO I=I1+1,I2
      ...
      IF (J>1)    CALL GET_BOUNDARY_TYPE
      J   = J1
      IC  = CELL_INDEX(I,J  ,K)
      ICG = CELL_INDEX(I,J+1,K)
      IOR = 2
      IW  = CELL(ICG)%WALL_INDEX(-2)
      IF (J<JBAR) CALL GET_BOUNDARY_TYPE
      J   = J2
      IC  = CELL_INDEX(I,J  ,K)
      ICG = CELL_INDEX(I,J+1,K)
      IOR = 2
...
DO J=J1+1,J2
   DO I=I1+1,I2
      ...
      IF (K>1)    CALL GET_BOUNDARY_TYPE
      K   = K1  
      IC  = CELL_INDEX(I,J,K  )
      ICG = CELL_INDEX(I,J,K+1)
      IOR = 3
      IW  = CELL(ICG)%WALL_INDEX(-3)
      IF (K<KBAR) CALL GET_BOUNDARY_TYPE
      K   = K2
      IC  = CELL_INDEX(I,J,K  )
      ICG = CELL_INDEX(I,J,K+1)
      IOR = 3
...
INTEGER :: IIG,JJG,KKG,IW_OLD,IERR,PRESSURE_BC_TYPE,ICG_OLD,II
...
IF (IW==0) THEN
   ...
   MESHES(NM)%N_INTERNAL_WALL_CELLS = MESHES(NM)%N_INTERNAL_WALL_CELLS + 1
   MESHES(NM)%N_WALL_CELLS = MESHES(NM)%N_EXTERNAL_WALL_CELLS + MESHES(NM)%N_INTERNAL_WALL_CELLS
   IW  = MESHES(NM)%N_WALL_CELLS
   CALL INIT_WALL_CELL(NM,I,J,K,OBST_INDEX,IW,IOR,OB%SURF_INDEX_INTERIOR,IERR)
   IF (IERR>0) RETURN
ENDIF
...
USE_OLD_WALL_CELL: IF (REMOVE .AND. ( (SF%THERMAL_BC_INDEX==THERMALLY_THICK.AND.(SF%VARIABLE_THICKNESS.OR.SF%HT_DIM>1)) &
   ...
   IF (MESHES(NM)%CELL(ICG_OLD)%EXTERIOR) THEN
      EWC => MESHES(NM)%EXTERNAL_WALL(CELL(ICG)%WALL_INDEX(BC%IOR))
      NOM = EWC%NOM
      IF (NOM>0) THEN
