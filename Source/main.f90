...
IF (OBST_CREATED_OR_REMOVED) THEN
   ...
   ENDDO
   IF (.NOT.INITIALIZATION_PHASE) CALL INITIALIZE_BACK_WALL_EXCHANGE
ENDIF

!CALL MPI_ALLREDUCE(MPI_IN_PLACE,REINITIALIZE_BACK_WALL_EXCHANGE,INTEGER_ONE,MPI_LOGICAL,MPI_LOR,MPI_COMM_WORLD,IERR)
!IF (REINITIALIZE_BACK_WALL_EXCHANGE) CALL INITIALIZE_BACK_WALL_EXCHANGE
!REINITIALIZE_BACK_WALL_EXCHANGE = .FALSE.
...
ENDDO
write(0,*) icyc,my_rank,' exits FIND_WALL_BACK'
...
ENDDO
write(0,*) icyc,my_rank,' exits ADJUST'
...
write(0,*) icyc,my_rank,' exits POST 8'
CALL MESH_EXCHANGE(8)
write(0,*) icyc,my_rank,' exits MESH 8'
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   DO NOM=1,NMESHES
      IF (NM==NOM) CYCLE
      DO ITER=1,2
         IF (ITER==1) OS => MESHES(NM)%OMESH(NOM)%WALL_SEND_BUFFER
         IF (ITER==2) OS => MESHES(NM)%OMESH(NOM)%THIN_WALL_SEND_BUFFER
         IF (OS%N_ITEMS>0) THEN
            ...
            ALLOCATE(OS%ITEM_INDEX(OS%N_ITEMS_DIM))
            ALLOCATE(OS%SURF_INDEX(OS%N_ITEMS_DIM))
         ENDIF
      ENDDO
   ENDDO
ENDDO
