...
TYPE (CELL_WEIGHT_TYPE), POINTER :: CW
...
SELECT CASE(TASK_NUMBER)
   ...
   CASE(5)
         ...
         DO NM=1,NMESHES
            DO NOM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
               IF (PROCESS(NM)/=MY_RANK .AND. MESHES(NOM)%CONNECTED_MESH(NM)) THEN
                  ...
                  ENDIF
                  CALL MPI_IRECV(M2%INTEGER_RECV_BUFFER(1),8,MPI_INTEGER,PROCESS(NM),NM,MPI_COMM_WORLD,REQ0(N_REQ0),IERR)
               ENDIF
            ENDDO
         ENDDO
      ...
      DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         DO NOM=1,NMESHES
            ...
            IF (PROCESS(NOM)/=MY_RANK) THEN
               ...
               M3%INTEGER_SEND_BUFFER(8) = M3%N_EWC_R
               N_REQ0 = N_REQ0 + 1
               IF(N_REQ0>SIZE(REQ0,DIM=1)) THEN
                  ALLOCATE(REQ0DUM(SIZE(REQ0,DIM=1)+NMESHES))
                  REQ0DUM(1:N_REQ0-1) = REQ0(1:N_REQ0-1)
                  CALL MOVE_ALLOC(REQ0DUM,REQ0)
               ENDIF
               CALL MPI_ISEND(M3%INTEGER_SEND_BUFFER(1),8,MPI_INTEGER,PROCESS(NOM),NM,MPI_COMM_WORLD,REQ0(N_REQ0),IERR)
            ELSE
               M2 => MESHES(NOM)%OMESH(NM)
         ...
         DO NM=1,NMESHES
            DO NOM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
               IF (NM/=NOM .AND. PROCESS(NM)/=MY_RANK .AND. MESHES(NOM)%CONNECTED_MESH(NM)) THEN
                  ...
                  M2%N_EWC_S = M2%INTEGER_RECV_BUFFER(8)
               ENDIF
            ENDDO
         ENDDO
      ...
      DO NM=1,NMESHES
         DO NOM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
            IF (MESHES(NOM)%OMESH(NM)%NIC_S>0) THEN
               ...
               ALLOCATE(M2%IIG_S(M2%N_EWC_S))
               ALLOCATE(M2%JJG_S(M2%N_EWC_S))
               ALLOCATE(M2%KKG_S(M2%N_EWC_S))
               ALLOCATE(M2%INT_S(M2%N_EWC_S))
               ALLOCATE(M2%IORE_S(M2%N_EWC_S))
            ENDIF
         ENDDO
      ENDDO
      ...
      DO NM=1,NMESHES
         DO NOM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
            ...
            IF (N_MPI_PROCESSES>1 .AND. NM/=NOM .AND. PROCESS(NM)/=MY_RANK .AND. M2%NIC_S>0) THEN
               ...
               CALL MPI_IRECV(M2%IIG_S(1),M2%N_EWC_S,MPI_INTEGER,PROCESS(NM),NM,MPI_COMM_WORLD,REQ(N_REQ+5),IERR)
               CALL MPI_IRECV(M2%JJG_S(1),M2%N_EWC_S,MPI_INTEGER,PROCESS(NM),NM,MPI_COMM_WORLD,REQ(N_REQ+6),IERR)
               CALL MPI_IRECV(M2%KKG_S(1),M2%N_EWC_S,MPI_INTEGER,PROCESS(NM),NM,MPI_COMM_WORLD,REQ(N_REQ+7),IERR)
               CALL MPI_IRECV(M2%IORE_S(1),M2%N_EWC_S,MPI_INTEGER,PROCESS(NM),NM,MPI_COMM_WORLD,REQ(N_REQ+8),IERR)
               CALL MPI_IRECV(M2%INT_S(1),M2%N_EWC_S,MPI_INTEGER,PROCESS(NM),NM,MPI_COMM_WORLD,REQ(N_REQ+9),IERR)
               N_REQ = N_REQ + 9
            ENDIF
         ENDDO
      ENDDO
      ...
      DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         DO NOM=1,NMESHES
            ...
            IF (PROCESS(NOM)/=MY_RANK) THEN
               ...
               CALL MPI_ISEND(M3%IIG_R(1),M3%N_EWC_R,MPI_INTEGER,PROCESS(NOM),NM,MPI_COMM_WORLD,REQ(N_REQ+5),IERR)
               CALL MPI_ISEND(M3%JJG_R(1),M3%N_EWC_R,MPI_INTEGER,PROCESS(NOM),NM,MPI_COMM_WORLD,REQ(N_REQ+6),IERR)
               CALL MPI_ISEND(M3%KKG_R(1),M3%N_EWC_R,MPI_INTEGER,PROCESS(NOM),NM,MPI_COMM_WORLD,REQ(N_REQ+7),IERR)
               CALL MPI_ISEND(M3%IORE_R(1),M3%N_EWC_R,MPI_INTEGER,PROCESS(NOM),NM,MPI_COMM_WORLD,REQ(N_REQ+8),IERR)
               CALL MPI_ISEND(M3%INT_R(1),M3%N_EWC_R,MPI_INTEGER,PROCESS(NOM),NM,MPI_COMM_WORLD,REQ(N_REQ+9),IERR)
               N_REQ = N_REQ + 9
            ELSE
               M2 => MESHES(NOM)%OMESH(NM)
               M2%IIO_S = M3%IIO_R
      ...
      ! Create weights for scalars packages

      DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         DO NOM=1,NMESHES
            M3 => MESHES(NM)%OMESH(NOM)
            IF (M3%N_EWC_S<1) CYCLE
            M  => MESHES(NM)
            M4 => MESHES(NOM)
            ALLOCATE(M3%CELL_WEIGHT(M3%N_EWC_S,2))
            DO N=1,M3%N_EWC_S
               CW => M3%CELL_WEIGHT(N,1)
               CALL FILL_WEIGHTING_ARRAY(1,N,CW)
               CW => M3%CELL_WEIGHT(N,2)
               CALL FILL_WEIGHTING_ARRAY(2,N,CW)
            ENDDO
         ENDDO
      ENDDO
...
SUBROUTINE FILL_WEIGHTING_ARRAY(STEP,N,CW)

INTEGER, INTENT(IN) :: STEP,N
REAL(EB) :: X1,X2,Y1,Y2,Z1,Z2,XI,YJ,ZK,FRAC
INTEGER :: NN,NN_DIM,I1,J1,K1,I2,J2,K2,I,J,K
TYPE(CELL_WEIGHT_TYPE), POINTER :: CW

! Determine the coordinates (X1,X2,Y1,Y2,Z1,Z2) of the ghost cell (STEP=1 is first ghost cell; STEP=2 is second)

X1=M4%X(M3%IIG_S(N)-1) ; X2=M4%X(M3%IIG_S(N))
Y1=M4%Y(M3%JJG_S(N)-1) ; Y2=M4%Y(M3%JJG_S(N))
Z1=M4%Z(M3%KKG_S(N)-1) ; Z2=M4%Z(M3%KKG_S(N))
SELECT CASE(M3%IORE_S(N))
   CASE( 1) ; X2=M%X(M3%INT_S(N))-(STEP-1)*M4%DX(M3%IIG_S(N)) ; X1=X2-M4%DX(M3%IIG_S(N))
   CASE(-1) ; X1=M%X(M3%INT_S(N))+(STEP-1)*M4%DX(M3%IIG_S(N)) ; X2=X1+M4%DX(M3%IIG_S(N))
   CASE( 2) ; Y2=M%Y(M3%INT_S(N))-(STEP-1)*M4%DY(M3%JJG_S(N)) ; Y1=Y2-M4%DY(M3%JJG_S(N))
   CASE(-2) ; Y1=M%Y(M3%INT_S(N))+(STEP-1)*M4%DY(M3%JJG_S(N)) ; Y2=Y1+M4%DY(M3%JJG_S(N))
   CASE( 3) ; Z2=M%Z(M3%INT_S(N))-(STEP-1)*M4%DZ(M3%KKG_S(N)) ; Z1=Z2-M4%DZ(M3%KKG_S(N))
   CASE(-3) ; Z1=M%Z(M3%INT_S(N))+(STEP-1)*M4%DZ(M3%KKG_S(N)) ; Z2=Z1+M4%DZ(M3%KKG_S(N))
END SELECT

CALL GET_IJK(X1,Y1,Z1,NM,XI,YJ,ZK,I1,J1,K1) ; I1=MAX(1,I1)      ; J1=MAX(1,J1)      ; K1=MAX(1,K1)
CALL GET_IJK(X2,Y2,Z2,NM,XI,YJ,ZK,I2,J2,K2) ; I2=MIN(M%IBAR,I2) ; J2=MIN(M%JBAR,J2) ; K2=MIN(M%KBAR,K2)

NN_DIM = (I2-I1+1)*(J2-J1+1)*(K2-K1+1)
ALLOCATE(CW%I(NN_DIM))
ALLOCATE(CW%J(NN_DIM))
ALLOCATE(CW%K(NN_DIM))
ALLOCATE(CW%WGT(NN_DIM))
NN = 0
DO K=K1,K2
   DO J=J1,J2
      DO I=I1,I2
         FRAC = (MIN(M%X(I),X2)-MAX(M%X(I-1),X1))/(X2-X1) * &
                (MIN(M%Y(J),Y2)-MAX(M%Y(J-1),Y1))/(Y2-Y1) * &
                (MIN(M%Z(K),Z2)-MAX(M%Z(K-1),Z1))/(Z2-Z1)
         IF (FRAC>0.0001_EB) THEN
            NN = NN + 1
            CW%I(NN) = I
            CW%J(NN) = J
            CW%K(NN) = K
...
END SUBROUTINE FILL_WEIGHTING_ARRAY
...
INTEGER :: IMIN,IMAX,JMIN,JMAX,KMIN,KMAX,NOM,IOR,IW,N,IIO,JJO,KKO,NIC_R,ILC,N_EWC_R
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   IF (OM%NIC_R>0) THEN
      ...
      ALLOCATE(OM%IIG_R(OM%N_EWC_R))
      ALLOCATE(OM%JJG_R(OM%N_EWC_R))
      ALLOCATE(OM%KKG_R(OM%N_EWC_R))
      ALLOCATE(OM%INT_R(OM%N_EWC_R))
      ALLOCATE(OM%IORE_R(OM%N_EWC_R))
      NIC_R = 0
      N_EWC_R = 0
      INDEX_LOOP: DO IW=1,M%N_EXTERNAL_WALL_CELLS
         ...
         OM%IIG_R(N_EWC_R) = M%BOUNDARY_COORD(M%WALL(IW)%BC_INDEX)%IIG
         OM%JJG_R(N_EWC_R) = M%BOUNDARY_COORD(M%WALL(IW)%BC_INDEX)%JJG
         OM%KKG_R(N_EWC_R) = M%BOUNDARY_COORD(M%WALL(IW)%BC_INDEX)%KKG
         OM%IORE_R(N_EWC_R) = IOR
         ...
         ENDDO
         SELECT CASE(IOR)
            CASE( 1) ; OM%INT_R(N_EWC_R) = EWC%IIO_MIN
            CASE(-1) ; OM%INT_R(N_EWC_R) = EWC%IIO_MIN-1
            CASE( 2) ; OM%INT_R(N_EWC_R) = EWC%JJO_MIN
            CASE(-2) ; OM%INT_R(N_EWC_R) = EWC%JJO_MIN-1
            CASE( 3) ; OM%INT_R(N_EWC_R) = EWC%KKO_MIN
            CASE(-3) ; OM%INT_R(N_EWC_R) = EWC%KKO_MIN-1
         END SELECT
      ENDDO INDEX_LOOP
    ...
    ! IF (RNODE==SNODE) CYCLE SENDING_MESH_LOOP
      ...
      INITIALIZATION_IF: IF (CODE==0) THEN
         ...
         IF (M3%NIC_R>0) THEN
            ...
            ALLOCATE(M3%REAL_RECV_PKG1(M3%N_EWC_R*(6+2*N_TOTAL_SCALARS)))
      ...
      IF (CODE==8 .AND. RNODE/=SNODE) THEN
         OS => M3%WALL_SEND_BUFFER
      ...
      IF (CODE==19 .AND. OS%N_ITEMS>0 .AND. RNODE/=SNODE) THEN
      ...
      OS => M3%PARTICLE_RECV_BUFFER
      IF (CODE==11 .AND. OS%N_ITEMS>0 .AND. RNODE/=SNODE) THEN
...
REAL(EB) :: TNOW,XI,YJ,ZK,WGT
INTEGER, INTENT(IN) :: CODE
INTEGER :: NM,NOM,I,II,JJ,KK,LL,LLL,N,RNODE,SNODE,IMIN,IMAX,JMIN,JMAX,KMIN,KMAX,IJK_SIZE,RC,IC,LC
INTEGER :: NNN,NN1,NN2,IPC,II1,II2,JJ1,JJ2,KK1,KK2,NQT2,NQ,NN,IOR,NRA,NRA_MAX,AIC,OBST_INDEX,IP,IW
REAL(EB), POINTER, DIMENSION(:,:) :: PHI_LS_P
REAL(EB), POINTER, DIMENSION(:,:,:) :: HP,HP2,RHOP,DP,UP,UP2,VP,VP2,WP,WP2
...
TYPE (WALL_TYPE), POINTER :: WC
TYPE (THIN_WALL_TYPE), POINTER :: TW
TYPE (CELL_WEIGHT_TYPE), POINTER :: CW1,CW2
...
SENDING_MESH_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   RECEIVING_MESH_LOOP: DO NNN=1,M%N_NEIGHBORING_MESHES
      ...
      M3 => M%OMESH(NOM)
      IF (NOM==NM .AND. M3%N_EWC_S==0) CYCLE RECEIVING_MESH_LOOP
      ...
      INITIALIZE_SEND_IF: IF (CODE==0) THEN
         ...
         IF (M3%NIC_S>0) THEN
            ...
            ALLOCATE(M3%REAL_SEND_PKG1(M3%N_EWC_S*(6+2*N_TOTAL_SCALARS)))
         ...
         IF (M3%N_EWC_S>0 .AND. RNODE/=SNODE) THEN
      ...
      IF ((CODE==1.OR.CODE==4) .AND. M3%N_EWC_S>0) THEN
         IF (CODE==1) THEN
         ...
         M3%REAL_SEND_PKG1 = 0._EB
         PACK_REAL_SEND_PKG1: DO LL=1,M3%N_EWC_S
            NQ = NQT2*(LL-1)
            CW1 => M3%CELL_WEIGHT(LL,1)
            CW2 => M3%CELL_WEIGHT(LL,2)
            DO N=1,CW1%N
               II = CW1%I(N)
               JJ = CW1%J(N)
               KK = CW1%K(N)
               WGT = CW1%WGT(N)
               M3%REAL_SEND_PKG1(NQ+1) = M3%REAL_SEND_PKG1(NQ+1) + RHOP(II,JJ,KK)*WGT
               M3%REAL_SEND_PKG1(NQ+3) = M3%REAL_SEND_PKG1(NQ+3) + M%MU(II,JJ,KK)*WGT
               M3%REAL_SEND_PKG1(NQ+4) = M3%REAL_SEND_PKG1(NQ+4) + M%KRES(II,JJ,KK)*WGT
               M3%REAL_SEND_PKG1(NQ+5) = M3%REAL_SEND_PKG1(NQ+5) + DP(II,JJ,KK)*WGT
               M3%REAL_SEND_PKG1(NQ+6) = M3%REAL_SEND_PKG1(NQ+6) + M%Q(II,JJ,KK)*WGT
               DO NN=1,N_TOTAL_SCALARS
                  M3%REAL_SEND_PKG1(NQ+6+2*NN-1) = M3%REAL_SEND_PKG1(NQ+6+2*NN-1) + ZZP(II,JJ,KK,NN)*WGT
               ENDDO
            ENDDO
            DO N=1,CW2%N
               II = CW2%I(N)
               ...
               KK = CW2%K(N)
               WGT = CW2%WGT(N)
               M3%REAL_SEND_PKG1(NQ+2) = M3%REAL_SEND_PKG1(NQ+2) +  RHOP(II,JJ,KK)*WGT
               DO NN=1,N_TOTAL_SCALARS
                  M3%REAL_SEND_PKG1(NQ+6+2*NN) = M3%REAL_SEND_PKG1(NQ+6+2*NN) + ZZP(II,JJ,KK,NN)*WGT
               ENDDO
            ENDDO
         ENDDO PACK_REAL_SEND_PKG1
         IF (RNODE==SNODE) MESHES(NOM)%OMESH(NM)%REAL_RECV_PKG1 = M3%REAL_SEND_PKG1
      ENDIF
...
RECV_MESH_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   SEND_MESH_LOOP: DO NNN=1,M%N_NEIGHBORING_MESHES
      ...
      IF ((CODE==1.OR.CODE==4) .AND. M2%N_EWC_R>0) THEN
            NQT2 = 6+2*N_TOTAL_SCALARS
            IF (CODE==1) THEN
               RHOP => M%RHOS ; DP => M%D  ; ZZP => M%ZZS
            ELSE
               RHOP => M%RHO  ; DP => M%DS ; ZZP => M%ZZ
            ENDIF
            UNPACK_REAL_RECV_PKG1: DO LL=1,M2%N_EWC_R
               II1 = M2%IIG_R(LL) ; II2=II1
               JJ1 = M2%JJG_R(LL) ; JJ2=JJ1
               KK1 = M2%KKG_R(LL) ; KK2=KK1
               SELECT CASE(M2%IORE_R(LL))
                  CASE(-1) ; II1=II1+1 ; II2=II1+1
                  CASE( 1) ; II1=II1-1 ; II2=II1-1
                  CASE(-2) ; JJ1=JJ1+1 ; JJ2=JJ1+1
                  CASE( 2) ; JJ1=JJ1-1 ; JJ2=JJ1-1
                  CASE(-3) ; KK1=KK1+1 ; KK2=KK1+1
                  CASE( 3) ; KK1=KK1-1 ; KK2=KK1-1
               END SELECT
                  RHOP(II1,JJ1,KK1) = M2%REAL_RECV_PKG1(NQT2*(LL-1)+1)
                  RHOP(II2,JJ2,KK2) = M2%REAL_RECV_PKG1(NQT2*(LL-1)+2)
                  M%MU(II1,JJ1,KK1) = M2%REAL_RECV_PKG1(NQT2*(LL-1)+3)
                M%KRES(II1,JJ1,KK1) = M2%REAL_RECV_PKG1(NQT2*(LL-1)+4)
                    DP(II1,JJ1,KK1) = M2%REAL_RECV_PKG1(NQT2*(LL-1)+5)
                   M%Q(II1,JJ1,KK1) = M2%REAL_RECV_PKG1(NQT2*(LL-1)+6)
               DO NN=1,N_TOTAL_SCALARS
