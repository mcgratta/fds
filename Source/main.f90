...
IF (OBST_CREATED_OR_REMOVED) THEN
   ...
   DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      ...
      IF (UBOUND(MESHES(NM)%WALL_WORK1,DIM=1)<MESHES(NM)%N_WALL_CELLS_DIM) THEN  ! reallocate work arrays
         DEALLOCATE(MESHES(NM)%WALL_WORK1)
         DEALLOCATE(MESHES(NM)%WALL_WORK2)
         ALLOCATE(MESHES(NM)%WALL_WORK1(MESHES(NM)%N_WALL_CELLS_DIM))
         ALLOCATE(MESHES(NM)%WALL_WORK2(MESHES(NM)%N_WALL_CELLS_DIM))
      ENDIF
   ENDDO
