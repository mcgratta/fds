...
IFC_LOOP : DO IFC=1,CC%CCELEM(1,JCC)
   ...
   END SELECT
ENDDO IFC_LOOP
IF(.NOT.RETURN_INTEGRALS) WHERE(ATOTV>TWO_EPSILON_EB) UVWAV = UVWAV / ATOTV
...
IF (PREDICTOR) THEN
   ...
   MESHES_LOOP1 : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      ...
      IF (.NOT.ALLOCATED(MESHES(NM)%RHO_ZZN)) ALLOCATE(MESHES(NM)%RHO_ZZN(-1:IBP1+1,-1:JBP1+1,-1:KBP1+1,N_TOTAL_SCALARS))
      DO N=1,N_TOTAL_SCALARS
   ...
   SIGN_LOOP: DO I_SGN=-1,1,2
      ...
      SELECT CASE(SF%VELOCITY_BC_INDEX)
         ...
         CASE (BOUNDARY_FUEL_MODEL_BC)
            ...
            ENDIF
            HT_SCALE_FACTOR = MIN(1._EB,VEG_HT/ABS(TWO_EPSILON_EB+2._EB*DXN_STRM_UB))
